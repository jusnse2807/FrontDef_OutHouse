...
const ErrorPopup: React.FC<ErrorPopupProps> = ({ message, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-red-700 text-white p-6 rounded-2xl shadow-2xl relative w-80 border border-red-500">
        <button
          onClick={onClose}
          className="absolute top-2 right-2 text-white font-bold hover:text-red-200"
        >
          &times;
        </button>
...
const LoginPopup: React.FC<ErrorPopupProps> = ({ message, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-green-700 text-white p-6 rounded-2xl shadow-2xl relative w-80 border border-green-500">
        <button
          onClick={onClose}
          className="absolute top-2 right-2 text-white font-bold hover:text-green-200"
        >
          &times;
        </button>
...
export const LoginEmpresa = () => {
  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    const url =
      "https://back-and-front-nigthlifemedellin.onrender.com/discotecas/login";
